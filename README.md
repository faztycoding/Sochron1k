...
costs from a strict labelled-trade bundle. The offline
[`sochron-pa01-backtest`](docs/operations/pa01-tick-replay-backtest.md) operator can
...
non-synthetic strategy-performance evidence and every trading path remain missing.
The separate replay and research evaluator are explicit offline/default-off tools,
are not started by Compose and cannot promote a strategy or enable execution.
...
recovery and historical tick/bar acquisition remain missing. The PA01 tick-replay
labeller and statistics evaluator/publication path now exist, but they have no
non-synthetic replay artifact and therefore cannot establish strategy performance.
