...
export const connectionDefinitions: Record<ConnectionId, Definition> = {
  ...
  demo_readiness: { title: "Gate ความพร้อม Demo", result: "ข้อมูลที่ขาด runtime และหลักฐานเป้าหมาย",
    routes: ["/api/ui/demo-readiness", "/api/owner/target-evidence"], required: null,
    sources: ["owner_decisions", "runtime_gates", "target_evidence_snapshot"],
    sourceLabel: "Redacted owner decisions + component states + normalized target evidence", implementation: "available" },
  ...
  execution_evidence: { title: "คำสั่งและการป้องกันสถานะ", result: "Order / Deal / Position / SL ที่ยืนยันแล้ว", routes: ["/api/executor/v1/status", "/api/owner/execution", "/api/internal/v1/demo-round-trip/status"],
    required: null, sources: ["mt5_execution", "local_execution_journal", "bounded_demo_round_trip_admission"],
    sourceLabel: "MT5 Execution EA + SQLite journal + bounded internal admission", implementation: "available" },
  operational_alerts: { title: "ศูนย์แจ้งเตือน", result: "เหตุสำคัญ coverage และ lifecycle ของ owner",
    routes: ["/api/owner/alerts", "/api/owner/alerts/{condition_id}/acknowledge", "/api/owner/alerts/{condition_id}/resolve", "/api/owner/api-budget"],
    required: "configured receipt-capable alert destination",
    sources: ["telemetry_status", "execution_status", "execution_journal", "bar_history", "policy_status", "alert_lifecycle", "api_budget_snapshot", "alert_delivery_outbox"],
    sourceLabel: "Runtime states + lifecycle journal + provider cost snapshot + durable delivery outbox", implementation: "partial" },
