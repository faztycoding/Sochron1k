...
export const connectionDefinitions: Record<ConnectionId, Definition> = {
  ...
  operational_alerts: { title: "ศูนย์แจ้งเตือน", result: "เหตุสำคัญ coverage และ lifecycle ของ owner",
    routes: ["/api/owner/alerts", "/api/owner/alerts/{condition_id}/acknowledge", "/api/owner/alerts/{condition_id}/resolve"],
    required: "provider budget source + external alert delivery",
    sources: ["telemetry_status", "execution_status", "execution_journal", "bar_history", "policy_status", "alert_lifecycle", "api_budget"],
    sourceLabel: "Runtime states + lifecycle journal · ยังไม่ส่งแจ้งเตือนภายนอก", implementation: "partial" },
